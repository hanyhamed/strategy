...
function GetX($p1) 
{
	...
	include "dbconfig.php";
	$sql="SELECT `index_name`,`index_value` FROM `indexes_months` WHERE `region`='".$p1."' AND `cat`='المؤشرات الأمنية و العسكرية' ORDER BY `month`";
	$res= mysqli_query($conn,$sql);
 $temp='" "';
 while($rr=$res->fetch_row())	
 {
...
function GetColors($p1) 
{
	...
	include "dbconfig.php";
	$sql="SELECT `index_name`,`index_value` FROM `indexes_months` WHERE `region`='".$p1."' AND `cat`='المؤشرات الأمنية و العسكرية' ORDER BY `id`";
	$res= mysqli_query($conn,$sql);
 $temp='"'.$colors[5].'"';
 $i=0;
 while($rr=$res->fetch_row())	
...
{
	include "dbconfig.php";
	$sql="SELECT `year`,`month`,`day`,`item`,`info` FROM `informations` WHERE `region`='".$_GET["country"]."'  AND `index_name`='المؤشرات الأمنية و العسكرية' ORDER BY `month_no` ,`day`";
...
function GetComment()
{
	$sql="SELECT `comment` FROM `comments` WHERE `region`='".$_GET["country"]."' AND `index_name`='المؤشرات الأمنية و العسكرية'";
	include "dbconfig.php";
		$res= mysqli_query($conn,$sql);
		$temp="";
		if($rr=$res->fetch_row())	
...
<div class="container">
<h1>المؤشرات الأمنية و العسكرية لدولة<?php echo " ".$_GET["country"]; ?></h1>
<?php
 ...
 echo 'new Chart("myChart", {
  ...
  options: {
    ...
    title: {
      display: true,
	text: "المؤشرات الأمنية و العسكرية لدولة '.$_GET["country"].'"
    }
  }
});';
 echo '</script>';
