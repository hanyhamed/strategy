...
function GetX($p1) 
{
	...
	include "dbconfig.php";
	$sql="SELECT `affecting_country`,`effect_strength` FROM `roles` WHERE `region`='".$p1."'";
	$res= mysqli_query($conn,$sql);
 $temp='" "';
 while($rr=$res->fetch_row())	
 {
...
{
	include "dbconfig.php";
	$sql="SELECT `affecting_country`,`role`,`information`,`info_date`,`effect_strength` FROM `roles` WHERE `region`='".$_GET["country"]."' ORDER BY `info_date`";
...
function GetComment()
{
	$sql="SELECT `comment` FROM `roles_comments` WHERE `region`='".$_GET["country"]."'";
	include "dbconfig.php";
		$res= mysqli_query($conn,$sql);
		$temp="";
		if($rr=$res->fetch_row())	
...
<div class="container">
<h1>الأدوار الدولية و الإقليمية للدول مع <?php echo " ".$_GET["country"]; ?></h1>
<?php
 ...
 echo 'new Chart("myChart", {
  ...
  options: {
    ...
    title: {
      display: true,
	text: "الأدوار الدولية و الإقليمية للدول مع '.$_GET["country"].'"
    }
...
<h2>هذه النتائج ظهرت بعد تطبيق المؤشرات على  المعلومات التي تم جمعها خلال فترة البحث على دولة <?php echo $_GET["country"];?></h2>
  ...
  <thead>
    <tr>
	  ...
	  <th style="font-size:1.2em;">الدولة</th>
      <th style="font-size:1.2em;">الدور</th>
      <th style="font-size:1.2em;">المعلومة</th>   
 	  <th style="font-size:1.2em;">التاريخ</th>
	  <th style="font-size:1.2em;">قوة التأثير</th>
