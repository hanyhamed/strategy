...
function GetX($p1) 
{
	...
	include "dbconfig.php";
	$sql="SELECT `index_name`,`index_value` FROM `indexes_months` WHERE `region`='".$p1."' AND `cat`='مؤشرات التهدئة (صراعات الأقليمية)' ORDER BY `month`";
	$res= mysqli_query($conn,$sql);
 $temp='" "';
 while($rr=$res->fetch_row())	
 {
...
function GetColors($p1) 
{
	...
	include "dbconfig.php";
	$sql="SELECT `index_name`,`index_value` FROM `indexes_months` WHERE `region`='".$p1."' AND `cat`='مؤشرات التهدئة (صراعات الأقليمية)' ORDER BY `id`";
	$res= mysqli_query($conn,$sql);
 $temp='"'.$colors[5].'"';
 $i=0;
 while($rr=$res->fetch_row())	
...
{
	include "dbconfig.php";
	$sql="SELECT `year`,`month`,`day`,`item`,`info` FROM `informations` WHERE `region`='".$_GET["country"]."' AND `index_name`='مؤشرات التهدئة (صراعات الأقليمية)' ORDER BY `month_no` ,`day`";
...
function GetComment()
{
	$sql="SELECT `comment` FROM `comments` WHERE `region`='".$_GET["country"]."' AND `index_name`='مؤشرات التهدئة (صراعات الأقليمية)'";
	include "dbconfig.php";
		$res= mysqli_query($conn,$sql);
		$temp="";
		if($rr=$res->fetch_row())	
...
<div class="container">
<h1>مؤشرات التهدئة (صراعات الأقليمية) لدولة<?php echo " ".$_GET["country"]; ?></h1>
<?php
 ...
 echo 'new Chart("myChart", {
  ...
  options: {
    ...
    title: {
      display: true,
	text: "مؤشرات التهدئة (صراعات الأقليمية) لدولة '.$_GET["country"].'"
    }
  }
});';
 echo '</script>';
